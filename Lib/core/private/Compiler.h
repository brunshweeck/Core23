//
// Created by Brunshweeck on 11/09/2023 17:48.
// Copyright (c) 2023
//
...
#elif defined(CORE_COMPILER_GNU_ONLY) && CORE_COMPILER_GNU >= 1000
...
CORE_WARNING_DISABLE_MSVC(4097) /* typedef-name 'identifier1' used as synonym for class-name 'identifier2' */
CORE_WARNING_DISABLE_MSVC(4706) /* assignment within conditional expression */
CORE_WARNING_DISABLE_MSVC(4355) /* 'this' : used in base member initializer list */
CORE_WARNING_DISABLE_MSVC(4710) /* function not inlined */
CORE_WARNING_DISABLE_MSVC(4530) /* C++ exception handler used, but unwind semantics are not enabled. Specify /EHsc */
...
#if !defined(CORE_MOC_RUN)
#if defined(CORE_COMPILER_GNU) && !defined(__cpp_exceptions)
#define CORE_NO_EXCEPTIONS
#endif
#elif defined(CORE_BOOTSTRAPPED)
#define CORE_NO_EXCEPTIONS
#endif
#endif
...
#define ENABLE_EQUALITY_OPERATORS(Class) \
        gbool operator==(const Class &object) const { return equals((const Object &)object); } \
        gbool operator!=(const Class &object) const { return !(*this == object); }

#define DISABLE_EQUALITY_OPERATORS(Object) \
        gbool operator==(const Object &object) const = delete; \
        gbool operator!=(const Object &object) const = delete;

#if __cpp_deduction_guides > 201665
#define CORE_TEMPLATE_TYPE_DEDUCTION 1
#else
#define CORE_TEMPLATE_TYPE_DEDUCTION 0
#endif
...
/**
 * determine offset of class fields.
 * Used on Unsafe class methods
 * <p>
 * use CORE_WARNING_DISABLE_INVALID_OFFSETOF to disable compiler warnings
 */
#ifndef CORE_OFFSETOF
#if __has_builtin(__builtin_offsetof)
#define CORE_OFFSETOF(class, field) ((glong)__builtin_offsetof (class, field));

#elif defined(offsetof)
#define CORE_OFFSETOF(class, field) ((glong) offsetof (class, field));
