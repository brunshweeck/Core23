...
namespace core {
    namespace charset {
        ISO8859_9::ISO8859_9() :
            Charset("ISO-8859-9", StringArray::of("iso8859_9", "8859_9", "iso-ir-148", "ISO_8859-9", "ISO_8859-9:1989", "ISO8859-9", "latin5", "l5", "ibm920", "ibm-920", "920", "cp920", "csISOLatin5")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool ISO8859_9::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<ISO8859_9>::hasInstance(cs);
        }

        CharsetDecoder &ISO8859_9::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &ISO8859_9::encoder() const {
        ...
        CharArray ISO8859_9::b2c = CharArray::of(
        ...
        CharArray ISO8859_9::c2b = CharArray(0x200);
        CharArray ISO8859_9::c2bIndex = CharArray(0x100);
        CharArray ISO8859_9::c2bNR = CharArray(0);
