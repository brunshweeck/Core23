...
#include "Windows1250.h"
#include <core/charset/private/SingleByte.h>

namespace core {
    namespace charset {
        Windows1250::Windows1250() : Charset("windows-1250", StringArray::of("cp1250", "cp5346")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool Windows1250::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<Windows1250>::hasInstance(cs);
        }

        CharsetDecoder &Windows1250::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &Windows1250::encoder() const {
            return Unsafe::allocateInstance<SingleByte::Encoder>(INSTANCE, c2b, c2bIndex, true);
        }

        Object &Windows1250::clone() const {
            return INSTANCE;
        }

        CharArray Windows1250::b2c = CharArray::of(
        ...
        CharArray Windows1250::c2b = CharArray(0x600);
        CharArray Windows1250::c2bIndex = CharArray(0x100);
        CharArray Windows1250::c2bNR = CharArray(0);

        // initialize after b2c, c2b, ... for calling SingleByte.initC2B
        Windows1250 Windows1250::INSTANCE{};
    } // charset
} // core