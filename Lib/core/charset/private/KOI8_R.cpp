...
namespace core {
    namespace charset {
        KOI8_R::KOI8_R() : Charset("KOI8-R", StringArray::of("koi8_r", "koi8", "cskoi8r")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool KOI8_R::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<KOI8_R>::hasInstance(cs);
        }

        CharsetDecoder &KOI8_R::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &KOI8_R::encoder() const {
            return Unsafe::allocateInstance<SingleByte::Encoder>(INSTANCE, c2b, c2bIndex, true);
        }

        Object &KOI8_R::clone() const {
            return INSTANCE;
        }

        CharArray KOI8_R::b2c = CharArray::of(
        ...
        CharArray KOI8_R::c2b = CharArray(0x500);
        CharArray KOI8_R::c2bIndex = CharArray(0x100);
        CharArray KOI8_R::c2bNR = CharArray(0);

        // initialize after b2c, c2b, ... for calling SingleByte.initC2B
        KOI8_R KOI8_R::INSTANCE{};
    } // charset
} // core