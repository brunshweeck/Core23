...
#include "Windows1253.h"
#include <core/charset/private/SingleByte.h>

namespace core {
    namespace charset {
        Windows1253::Windows1253() : Charset("windows-1253", StringArray::of("cp1253", "cp5346")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool Windows1253::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<Windows1253>::hasInstance(cs);
        }

        CharsetDecoder &Windows1253::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &Windows1253::encoder() const {
            return Unsafe::allocateInstance<SingleByte::Encoder>(INSTANCE, c2b, c2bIndex, true);
        }

        Object &Windows1253::clone() const {
            return INSTANCE;
        }

        CharArray Windows1253::b2c = CharArray::of(
                0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021,      // 0x80 - 0x87
                0xFFFD, 0x2030, 0xFFFD, 0x2039, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD,      // 0x88 - 0x8f
                0xFFFD, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022, 0x2013, 0x2014,      // 0x90 - 0x97
                0xFFFD, 0x2122, 0xFFFD, 0x203A, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD,      // 0x98 - 0x9f
                0x00A0, 0x0385, 0x0386, 0x00A3, 0x00A4, 0x00A5, 0x00A6, 0x00A7,      // 0xa0 - 0xa7
                0x00A8, 0x00A9, 0xFFFD, 0x00AB, 0x00AC, 0x00AD, 0x00AE, 0x2015,      // 0xa8 - 0xaf
        ...
        CharArray Windows1253::c2b = CharArray(0x600);
        CharArray Windows1253::c2bIndex = CharArray(0x100);
        CharArray Windows1253::c2bNR = CharArray(0);

        // initialize after b2c, c2b, ... for calling SingleByte.initC2B
        Windows1253 Windows1253::INSTANCE{};
    } // charset
} // core