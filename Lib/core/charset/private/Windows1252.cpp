...
namespace core {
    namespace charset {
        Windows1252::Windows1252() : Charset("windows-1252", StringArray::of("cp1252", "cp5346")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool Windows1252::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<Windows1252>::hasInstance(cs);
        }

        CharsetDecoder &Windows1252::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &Windows1252::encoder() const {
            return Unsafe::allocateInstance<SingleByte::Encoder>(INSTANCE, c2b, c2bIndex, true);
        }

        Object &Windows1252::clone() const {
            return INSTANCE;
        }

        CharArray Windows1252::b2c = CharArray::of(
        ...
        CharArray Windows1252::c2b = CharArray(0x600);
        CharArray Windows1252::c2bIndex = CharArray(0x100);
        CharArray Windows1252::c2bNR = CharArray(0);

        // initialize after b2c, c2b, ... for calling SingleByte.initC2B
        Windows1252 Windows1252::INSTANCE{};
    } // charset
} // core