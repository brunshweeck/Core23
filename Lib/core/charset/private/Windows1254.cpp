...
#include "Windows1254.h"
#include <core/charset/private/SingleByte.h>

namespace core {
    namespace charset {
        Windows1254::Windows1254() : Charset("windows-1254", StringArray::of("cp1254", "cp5346")) {
            SingleByte::initC2B(b2c, c2bNR, c2b, c2bIndex);
        }

        gbool Windows1254::contains(const Charset &cs) const {
            return cs.name().equals("US-ASCII") || Class<Windows1254>::hasInstance(cs);
        }

        CharsetDecoder &Windows1254::decoder() const {
            return Unsafe::allocateInstance<SingleByte::Decoder>(INSTANCE, b2c, true, false);
        }

        CharsetEncoder &Windows1254::encoder() const {
            return Unsafe::allocateInstance<SingleByte::Encoder>(INSTANCE, c2b, c2bIndex, true);
        }

        Object &Windows1254::clone() const {
            return INSTANCE;
        }

        CharArray Windows1254::b2c = CharArray::of(
                0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021,      // 0x80 - 0x87
                0x02C6, 0x2030, 0x0160, 0x2039, 0x0152, 0xFFFD, 0xFFFD, 0xFFFD,      // 0x88 - 0x8f
                0xFFFD, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022, 0x2013, 0x2014,      // 0x90 - 0x97
                0x02DC, 0x2122, 0x0161, 0x203A, 0x0153, 0xFFFD, 0xFFFD, 0x0178,      // 0x98 - 0x9f
        ...
        CharArray Windows1254::c2b = CharArray(0x600);
        CharArray Windows1254::c2bIndex = CharArray(0x100);
        CharArray Windows1254::c2bNR = CharArray(0);

        // initialize after b2c, c2b, ... for calling SingleByte.initC2B
        Windows1254 Windows1254::INSTANCE{};
    } // charset
} // core